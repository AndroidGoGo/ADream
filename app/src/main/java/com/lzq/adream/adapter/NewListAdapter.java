...
import com.lzq.adream.activity.NewsDetailActivity;
...
import com.lzq.adream.utils.DisplayUtil;
 ...
 * on 2016.09.10:49
 */
public class NewListAdapter extends MultiItemRecycleViewAdapter<NewsList.NewslistBean>
{
    public static final int TYPE_ITEM = 0;
    public static final int TYPE_PHOTO_ITEM =1;

    public NewListAdapter(Context context, final List<NewsList.NewslistBean> datas)
    {
        super(context, datas, new MultiItemTypeSupport<NewsList.NewslistBean>()
        {
            ...
            @Override
            public int getItemViewType(int position, NewsList.NewslistBean msg)
            {
                if (!TextUtils.isEmpty(msg.getDescription()))
                {
                    return TYPE_ITEM;
                }
    ...
    @Override
    public void convert(ViewHolderHelper holder, NewsList.NewslistBean newsSummary) {
     ...
     * @param position
     */
    private void setItemValues(final ViewHolderHelper holder, final NewsList.NewslistBean newsSummary, final int position) {
        String title = newsSummary.getTitle();
        if (title == null) {
            title = newsSummary.getTitle();
        }
        String ptime = newsSummary.getCtime();
        String digest = newsSummary.getDescription();
        final String imgSrc = newsSummary.getPicUrl();
        ...
        holder.setOnClickListener(R.id.rl_root, new View.OnClickListener() {
            ...
            public void onClick(View view) {
               NewsDetailActivity.startAction(mContext,holder.getView(R.id.news_summary_photo_iv),newsSummary.getUrl(),imgSrc,newsSummary.getTitle());
            }
        });
    }
     ...
     * @param position
     */
    private void setPhotoItemValues(ViewHolderHelper holder, final NewsList.NewslistBean newsSummary, int position) {
        String title = newsSummary.getTitle();
        String ptime = newsSummary.getCtime();
        holder.setText(R.id.news_summary_title_tv,title);
        holder.setText(R.id.news_summary_ptime_tv,ptime);
    ...
    private void setImageView(ViewHolderHelper holder, NewsSummary newsSummary) {
        ...
        if (newsSummary.getAds() != null) {
            ...
            if (size >= 3) {
                ...
                holder.setText(R.id.news_summary_title_tv, App.getAppContext()
