...
import com.lzq.adream.utils.SetterExclusionStrategys;
...
public class RetrofitService {
    ...
    private RetrofitService(int type){
        if (retrofit == null) {
            synchronized (RetrofitService.class) {
                if (retrofit == null) {
                    initOkHttpClient();
    ...
    public static ApiService createAPI(int hostType) {
        RetrofitService  retrofitService   =  mNetManagers.get(hostType);
        if (retrofitService==null){
            retrofitService = new RetrofitService(hostType);
